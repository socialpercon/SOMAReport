...
public class SearchController {
	...
	public Response elastic_index(@PathParam("target") String target,
		...
		try {
			...
			if (!valid_target.contains(target) || target.equals("")
					|| target == null || jo == null) {
	...
	@Produces(MediaType.APPLICATION_JSON + ";charset=utf-8")
	public Response elastic_search(@PathParam("target") String target,
		...
		try {
			...
			if (!valid_target.contains(target) || target.equals("")
					|| target == null || query.equals("") || query == null) {
				return Response.status(500).type(MediaType.APPLICATION_JSON)
						.build();
			}
			...
			if (target.equals("report")) {
				url = elastic_base_url + "/somareport/" + target
						+ "/_search?q=_source.topic="
						+ URLEncoder.encode(query, "utf-8");
			} else if (target.equals("user")) {
				url = elastic_base_url + "/somareport/" + target
						+ "/_search?q=_source.name="
						+ URLEncoder.encode(query, "utf-8");
			} else if (target.equals("project")) {
				url = elastic_base_url + "/somareport/" + target
						+ "/_search?q=_source.title="
						+ URLEncoder.encode(query, "utf-8");
			}
	 ...
	 *************************************************/
	@POST
	@Path("/elastic_update/{target}/{_id}")
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON + ";charset=utf-8")
	public Response elastic_update(@PathParam("target") String target,
			JSONObject jo, @PathParam("_id") String _id) {
		...
		try {
			...
			String url = elastic_base_url + "/somareport/" + target + "/" + _id
					+ "/_update";
			URL obj = new URL(url);
			HttpURLConnection con = (HttpURLConnection) obj.openConnection();
			con.setDoOutput(true);
