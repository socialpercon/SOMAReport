...
public class GrizzlyServerBuilder {
	...
	public GrizzlyServerBuilder listener(
			final Class<? extends EventListener> listener) {
		this.listener = listener;
		return this;
	}

	public GrizzlyServerBuilder addInitParameter(final String name,
			final String value) {
		this.initParameters.put(name, value);
		return this;
	}

	public GrizzlyServerBuilder addInitParameters(
	...
	public void start() throws Exception {
		...
		System.out.println(String.format(
				"Jersey application started at %s\nHit enter to stop it...",
				url));
		System.in.read();
		server.shutdownNow();
	}
	...
	private void registerServlet(final WebappContext webapp) {
		ServletRegistration registration = webapp.addServlet(
				servlet.getSimpleName(), servlet);
	...
	private void registerFilter(final WebappContext webapp) {
		FilterRegistration registration = webapp.addFilter(
				filter.getSimpleName(), filter);
		registration.addMappingForUrlPatterns(EnumSet.of(
				DispatcherType.REQUEST, DispatcherType.INCLUDE,
				DispatcherType.FORWARD, DispatcherType.ERROR), mapping);
		registration.setInitParameters(initParameters);
	}
